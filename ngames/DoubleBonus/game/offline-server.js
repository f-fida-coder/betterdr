/*
 * offline-server.js — self-contained offline backend for Double Bonus Poker
 * (VP_Classic_D engine, VPDB).
 *
 ...
 * file hooks jQuery's $.ajax and answers those endpoints locally, so the
 * untouched original UI/engine runs offline and can be dropped into any site.
 *
 * Double Bonus Poker: a Jacks-or-Better game (no wild card) with three separate
 * four-of-a-kind tiers.  The pay table is the game's own captured table, so the
 * win math matches the original.  Overridable via window.VP_OFFLINE_CONFIG.
 *
 ...
 *                  allowedcoinvalues, allowedcoinvaluesd, newbalance, gameid=0
...
(function () {
    ...
    var PAYTABLE = CFG.paytable || {
        ...
        FL:  [6, 12, 18, 24, 30],
        FH:  [9, 18, 27, 36, 45],
        _4K: [50, 100, 150, 200, 250],   // four 5s..Kings
        SF:  [50, 100, 150, 200, 250],
        _44: [80, 160, 240, 320, 400],   // four 2s/3s/4s
        NR:  [125, 250, 375, 500, 2000], // natural royal
        _4A: [160, 320, 480, 640, 800]   // four aces
    };
    // order the pay grid is emitted in (matches the captured GetGameData)
    var HAND_ORDER = ['_2P', 'JB', '_3K', 'ST', 'FL', 'FH', '_4K', 'SF', '_44', 'NR', '_4A'];

    var BAL_KEY = 'vp_db_bal', GID_KEY = 'vp_db_gid', RND_KEY = 'vp_db_round';
    var mem = {};
    ...
    function nextGid() { var g = parseInt(load(GID_KEY), 10); if (isNaN(g)) g = 44100000; g += 1; store(GID_KEY, String(g)); return g; }
    ...
    function evalHand(idxs) {
        ...
        if (counts[0] === 4) {
            ...
            if (quad === 14) return '_4A';
            if (quad === 2 || quad === 3 || quad === 4) return '_44';
            return '_4K';
        }
    ...
    function install($) {
        ...
        $.__vpOffline = true;
        if (window.console) console.log('%c[offline] double bonus poker backend active', 'color:#28a745');
        return true;
    }
