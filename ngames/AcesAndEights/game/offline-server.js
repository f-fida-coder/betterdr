/*
 * offline-server.js — self-contained offline backend for Aces & Eights video
 * poker (VP_Classic_D engine, VPA8).
 *
 ...
 *
 * Aces & Eights: a Jacks-or-Better game (no wild) with bonus four-of-a-kinds:
 * four Aces or Eights (A8), four 7s (_47) and all other quads (_4K).
 *
...
(function () {
    ...
    var HAND_ORDER = ['JB', '_2P', '_3K', 'ST', 'FL', 'FH', '_4K', '_47', 'SF', 'A8', 'NR'];

    var BAL_KEY = 'vp_a8_bal', GID_KEY = 'vp_a8_gid', RND_KEY = 'vp_a8_round';
    var mem = {};
    ...
    function nextGid() { var g = parseInt(load(GID_KEY), 10); if (isNaN(g)) g = 46100000; g += 1; store(GID_KEY, String(g)); return g; }
    ...
    function evalHand(idxs) {
        ...
        if (uniq.length === 5) {
            ...
            else if (uniq[0] === 14 && uniq[1] === 5 && uniq[4] === 2) straight = true;
        }
        ...
        if (counts[0] === 2) {
            ...
            return (pr === 11 || pr === 12 || pr === 13 || pr === 14) ? 'JB' : '-';
        }
        return '-';
    }
    ...
    function install($) {
        ...
        $.__vpOffline = true;
        if (window.console) console.log('%c[offline] aces & eights video poker backend active', 'color:#28a745');
        return true;
    }
