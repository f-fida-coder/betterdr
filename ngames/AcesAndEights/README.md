# Aces & Eights — offline / embeddable video poker

A self-contained Aces & Eights video poker game (52-card deck, no wild). It runs
the original game engine and artwork with a local JavaScript backend
...
```html
<iframe src="/AcesAndEights/index.html"
...
table. Aces & Eights rewards four-of-a-kinds with three tiers.
...
| Four Aces or Eights | 80 | 400 |
| Four Sevens | 50 | 250 |
| Straight Flush | 50 | 250 |
| Four of a Kind (rest) | 20 | 100 |
| Full House | 7 | 35 |
| Flush | 5 | 25 |
| Straight | 4 | 20 |
...
- **GetGameData** → the pay grid `<hand>_<coins>` (e.g. `A8_5=400`, `_47_5=250`),
...
- Same VP_Classic_D engine as the other video-poker variants; only the pay table
  and four-of-a-kind tiers differ.
- Balance persists in `localStorage` (`vp_a8_*`); set `persist:false` for a fresh
