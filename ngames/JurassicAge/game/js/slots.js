...
var slotMachine = {
	...
	secondReelStopTime: 100, // since first reel's stop time, not since animation beginning
	thirdReelStopTime: 100, // since second reel's stop time, not since animation beginning
	...
	//----------------------------------------------------
 	fnStopReelsAndEndSpin : function(spinData2) {
 			if(isInFreeSpins == 0 /*&& (typeof(Global.Connector.frees) != "undefined" && Global.Connector.frees != null && Global.Connector.frees != ""  && Global.Connector.frees == "0")*/){
 				...
 				if(fastPlay){
	 				var fastPlayTimer2 = setTimeout(function(){clearTimeout(fastPlayTimer2); fastPlayTimer2 = null; processStop();}, 10);
	...
	spin: function() {
		...
		if(!fastPlay){
			...
			timeIncrement = 40;
			tickDelay = 20;
			slotMachine.secondReelStopTime = 100; // since first reel's stop time, not since animation beginning
			slotMachine.thirdReelStopTime=  100;
			flagHitsEval = false;
			animationsFrames = 20;
			opacityReelTime= 300;
			slotMachine.bounceTime = 1000;
		}else{
			setFastValues();
		}
...
//FITO
							if(WinnerLines != null && WinnerLines.length > 0){
								var maxHits = Object.keys(GroupLines).length;
	    						var counterHits = 0;
								showLinesAfertIncrease = setInterval(function(){
									...
									if(counterHits >= maxHits){
										counterHits= 0;
									}

									var currentKey = Object.keys(GroupLines)[counterHits];
    								var groupArrayHit = GroupLines[currentKey];

									var symbols = groupArrayHit[0];
									var coinWon = groupArrayHit[1];
									var winBoxToShow = groupArrayHit[2];
									var pathArray = groupArrayHit[3];

									for(var i = 0; i < pathArray.length; i++){
										var tempArrPath = pathArray[i];
										processHightLightIcons(processData, tempArrPath, symbols);
									}

									var displaYPayValue = coinWon * Global.Connector.cv;
									showWinningLines(winBoxToShow, formatWithThousandsNoDecimalZeros(displaYPayValue));
	...
	_start_reel_spin: function(i, timeOffset) {
		...
		elReel.addClass(shakeClass);
		elReel.addClass("shake-constant");

	},
	_stop_reel_spin: function(i, outcome) {
		var elReel = $('#reel' + i); // cache for performance
		elReel.removeClass(shakeClass);
		elReel.removeClass("shake-constant");

		console.log("llamando _stop_reel_spin");
		...
		elReel.animate({ top: 600}, 800,'easeInCubic', function() {
			//slotMachine.sounds['reel-stop'].play();
			elReel.css("top", "-600px");

			setReelIcons(i);

			setTimeout(function(){
					elReel.addClass("shake-slow");
					elReel.addClass("shake-constant");
					elReel.animate({top: 0}, 800, 'easeOutBounce', function(){

					elReel.removeClass("shake-slow");
					elReel.removeClass("shake-constant");
				});
			}, 1);
	...
	_stop_reel_spinBTN: function(i, outcome) {
		...
		/*elReel.animate({ top: 600}, 10, function() {
			elReel.css("top", "-600px");
			setReelIcons(i);
		});*/
		/*elReel.css("top", -600);
		var setResult = setTimeout(function(){
			clearTimeout(setResult);
			setResult = null;
			elReel.css("top", 0);
			setReelIcons(i);
		}, 200);*/
		...
		setReelIcons(i);
		elReel.animate({ top: 0}, 00,'easeInCubic', function() {
			//elReel.css("top", "-600px");
			setReelIcons(i);
		});
...
function setReelIcons(index){
	...
	if(index == null){
		for(var i= 0; i < 5; i++){
			$("#reel-"+(i + 1)+"-poss-1 img").attr('src',"images/icons/"+arrReelsClean[i][0]+".png");
			$("#reel-"+(i + 1)+"-poss-2 img").attr('src',"images/icons/"+arrReelsClean[i][1]+".png");
			$("#reel-"+(i + 1)+"-poss-3 img").attr('src',"images/icons/"+arrReelsClean[i][2]+".png");
		}
	}
	else{
		$("#reel-"+index+"-poss-1 img").attr('src',"images/icons/"+arrReelsClean[index-1][0]+".png");
		$("#reel-"+index+"-poss-2 img").attr('src',"images/icons/"+arrReelsClean[index-1][1]+".png");
		$("#reel-"+index+"-poss-3 img").attr('src',"images/icons/"+arrReelsClean[index-1][2]+".png");
...
function createDynamicReel(ReelsArr){
	...
	for(var i = 0; i < ReelsArr.length; i++){
		...
		$("#reel" + (parseInt(i) + 1)).height((3 * slotMachine.iconSize));
	}
}
...
function loadSlotMachine(){
	...
	//ServerManager.doInitAction(initMachine)
	//TEST LINE
...
function hideAllWinnerLines() {
	//$(".high-light-icon").hide();
	for (var i = 1; i <= boxesWinAllWay; i++) hideWinnerLine(i);
}

function hideWinnerLine(lineNumber) {
    $("#credits-box" + lineNumber).text("00");
    $("#credits-box" + lineNumber).hide();
}
...
function evaluteHits(){
	...
	for(var i = 0; i < maxHits; i++){
		var hitsArray = WinnerLines[i].split(":");
		var winBoxNumber = hitsArray[0];
		var winAmountFormat = hitsArray[1];
		var path = hitsArray[2];
		var iconsWin = hitsArray[3];
		var coinsWon = hitsArray[4];

		if(GroupLines[iconsWin] == null){
			GroupLines[iconsWin] = [iconsWin, coinsWon, winBoxNumber, [path]];
		}else{
			var partialArry = GroupLines[iconsWin];
			var arrayPaths = partialArry[3];
			var currentCoins = partialArry[1];
			if(arrayPaths.indexOf(path) < 0){
				arrayPaths.push(path);
				var totalCoins = parseInt(currentCoins) + parseInt(coinsWon);
				//console.log("currentCoins", iconsWin, totalCoins, currentCoins, coinsWon)
				GroupLines[iconsWin] = [iconsWin, totalCoins, winBoxNumber, arrayPaths];
			}
		}
	}

	//GroupLines[iconsWin] = [iconsWin, coinsWon, winBoxNumber, [path]];
	//GroupLines[5A] = [5A, 100, 10, [11231,23321,22222]];

/*for (var key in GroupLines) {
  console.log("key " + key + " has value " + GroupLines[key]);
}*/
//FITO	
	maxHits = Object.keys(GroupLines).length;
    linesInterVal = setInterval(function(){ 
    	if(WinnerLines != null && WinnerLines.length > 0 && counterHits < maxHits){
    		...
    		//groupArrayHit[iconsWin] = [iconsWin, coinsWon, winBoxNumber, [path]];
			//groupArrayHit[5A] = [5A, 100, 10, [11231,23321,22222]];

    		//console.log("currentKey", currentKey, groupArrayHit, processData)
    		...
    		/*for(var i = 0; i < WinnerLines.length; i++){
    			...
    			$(".reel-icon").css('opacity', '1');
    			if(showIconsBorder)
    				$(".reel-icon").removeClass('icon-border');
    		}*/
...
function processHightLightIcons(reelPoss, path, icons){
	...
	for(var i=0; i< totalIcons; i++){
		...
		var reelStripPos = i + 1;
		
		//var elementPos = parseInt(reelStripPos) + (parseInt(reelIconPos));

		var iconImageArr = $("#reel-"+ (parseInt(i) + 1 )+"-poss-" + reelIconPos).children().attr('src').split('/');
		var inconName = iconImageArr[iconImageArr.length-1].replace(".png", '');


		if(animatedIconsArrOr.indexOf(inconName)>= 0){
			$("#reel-"+ (parseInt(i) + 1 )+"-poss-" + reelIconPos).css('opacity', '0');			
		}else{
			$("#reel-"+ (parseInt(i) + 1 )+"-poss-" + reelIconPos).css('opacity', '1');
		}
	...
	if(animatedIconsArr.indexOf(icon)>= 0){
		...
		if(iconsAnimationsInterval["#high-light-icon-re"+reel+"-ro"+row] ==  null){
			...
			var tempInterval = setInterval(function(){
				...
				$("#high-light-icon-re"+reel+"-ro"+row).css('background-position', (bgPos -3 ) + 'px -3px');
				imageCounter++;
...
function getWinBoxNumber(reel, row){
	var combination = reel + "" + row;

	//console.log("combination", combination, reel, row)
	switch(combination){
...
function getWinnerLinesByPath(partPath, coins, symbols) {
	...
	//console.log("getWinnerLinesByPath   partPath", partPath, "symbols", symbols)
	...
	var reel = symbols[0];
	partPath += "";
	var row = partPath[parseInt(reel) - 1];

	var winBoxNumber= getWinBoxNumber(reel, row) ;
	
	retorno =  winBoxNumber+ ":" + formatWithThousandsNoDecimalZeros(displaYPayValue) + ":" + partPath + ":" + symbols + ":" + coins; 
    return retorno;
}
...
function bigWinAnimation(totalWin){
//#big-win-container
	var totalImages = 23;
	var currentImage  = 1;
	$("#big-win-container").show();
	var bigWinInterVal = setInterval(function(){
		$('#big-win-container').css('background-image','url(images/bigwin/explotion'+currentImage+'.png)');
		currentImage++;
		
		if(currentImage >= totalImages){
			//$("#big-win-container").hide();
			clearInterval(bigWinInterVal);
			bigWinInterVal = null;
			$('#big-win-container').css('background-image','');
		}
...
var requestAni = null;
var lastTimeCall = null;
function render() {
	var curDate = new Date();
	//console.log("RENDER ", lastTimeCall,curDate,  Math.abs(lastTimeCall - curDate))
	//if(lastTimeCall == null || (lastTimeCall != null && (Math.abs(lastTimeCall - curDate))> 100)){
...
smokeImage.src = "images/other/smoke-particle.png";
...
function processStop(){
	console.log("llamando a processStop  flagHitsEval", flagHitsEval)
	if(fastPlay){
		$("#spinButton").hide();
	}
	...
	$("#stopButton").hide();
	if(fastPlay){
		var showSPINTime = setTimeout(function(){clearTimeout(showSPINTime); showSPINTime = null;
			if(isInFreeSpins == 0)
				$("#spinButton").show();
		}, 200);	
	}else{
...
function setFastValues(){
	animationLinesTime = 250;
	timeToSpecialHits = 10;
	timeToPrcessFreeSpins = 500;
	noHitsTimer = 1;

	hitsIterationTime = 250;
