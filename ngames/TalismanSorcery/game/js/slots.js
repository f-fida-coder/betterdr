...
function createDynamicReel(ReelsArr){
	...
	for(var i = 0; i < ReelsArr.length; i++){
        ...
        ReelsArr[i] = ReelsArr[i]  + reelIcon[0] + reelIcon[1] + reelIcon[2] + reelIcon[3];
...
function evaluteHits(){
	...
	for(var i = 0; i < HitsArray.length; i++){
		...
		if($.isNumeric( hitArr[0])){
			...
			var LineWin = pathCoins * Global.Connector.cv;

			var reelRes1 = getWinnerLinesByPath(pathNormalLine, pathCoins, symbols);
			if(reelRes1.indexOf(";") >= 0){
				var splitArr = reelRes1.split(";");
				var tempRes2 = "";
				for(var tempI =0; tempI < splitArr.length; tempI++){
					if(tempRes2 == ""){
						tempRes2 = splitArr[tempI] + (':' + LineWin);
					}else{
						tempRes2 += (";" + splitArr[tempI] + (':' + LineWin));
					}
				...
				reelRes1 = reelRes1+ (':' + LineWin);
			}

			if(totalLineAndPrize == ""){


				totalLineAndPrize = reelRes1;
			}
			else{
				totalLineAndPrize += ";" + reelRes1;	
			}

			//console.log("index LineWin pathCoins totalLineAndPrize",i, LineWin , pathCoins, totalLineAndPrize);
		} 
		else //FreeSpins
		{
...
//console.log("WinnerLines", WinnerLines)
    var tempWin = (acumulateTotalWin== null? 0:acumulateTotalWin);
    linesInterVal = setInterval(function(){ 
    	if(WinnerLines != null && WinnerLines.length > 0 && counterHits < maxHits){
    		...
    		//console.log("tempWin", tempWin, hitArray[4], hitArray);
    		$('#lastWin').html(formatWithThousandsNoDecimalZeros(tempWin)); 
...
function processHightLightIcons(reelPoss, path, icons){
	...
	for(var i=0; i< totalIcons; i++){
		...
		if(showIconsBorder)
			$("#reel-"+ (parseInt(i) + 1 )+"-poss-" + elementPos).addClass('icon-border');
	}
*/
	var reelspossClean = reelPoss.replaceAll('"','');
	var ArrReeslPoss = reelspossClean.split(",");
...
function processStop(){
	...
	if(!flagHitsEval){
		...
		clearInterval(animateWinInterval);
		animateWinInterval = null;
		console.log("fastPlay fastPlay ", fastPlay)
		if(fastPlay){
			setTimeout(function(){
				evaluteHits();
			},300);
	...
	$("#stopButton").hide();
	if(fastPlay){
		var showSPINTime = setTimeout(function(){clearTimeout(showSPINTime); showSPINTime = null;$("#spinButton").show();}, 300);	
	}else{
		$("#spinButton").show();
	}
