/*
 * rl-game.js — American Roulette game engine (offline build)
 *
 * The original casino engine was not present in the capture (only its assets,
 * LoaderConfig.json, web-connector.js and the server responses were), so this
 * is a faithful re-implementation on top of the original artwork and sounds.
 * It speaks the same Init/Spin/Heartbeat protocol via window.Server
 * (see mock-server.js).
 *
...
(function () {
    ...
    // per-position maximums (mirrors Init.aspx max_* fields)
    var MAXPOS = { su: 25, sp: 50, st: 75, tz: 75, cn: 100, ff: 125, sx: 150,
                   dz: 100, cl: 100, lo: 100, hi: 100, ev: 100, od: 100, rd: 100, bk: 100 };

    var GAME_SESSION = 'offline|73|1|1|OFFLINE|en';
    var ROOT = '../../../aspnet/';

    // ------------------------------------------------------------------ state
    var balance = 0, minb = 1, maxb = 100, lastGameId = 0;
    ...
    try { history = JSON.parse(localStorage.getItem('rl_offline_history')) || []; }
    catch (e) { history = []; }
    ...
    function pushHistory(tok) {
        ...
        try { localStorage.setItem('rl_offline_history', JSON.stringify(history)); } catch (e) {}
        redrawHistory();
    }
    ...
    function doInit() {
        var params = 'GameSession=' + GAME_SESSION + '&lastgameid=0&rand=' +
                     Math.floor(Math.random() * 10000 + 1);
        window.Server.post(ROOT + 'roulette/Init.aspx', params, function (data) {
            var r = parseKv(data);
            balance = parseFloat(r.BAL) || 0;
            minb = parseFloat(r.MINB) || 1;
            maxb = parseFloat(r.MAXB) || 100;
    ...
    function doSpin() {
        ...
        var params = 'GameSession=' + GAME_SESSION + '&iseuro=null' + bets +
                     '&lastgameid=' + lastGameId + '&rand=' +
                     Math.floor(Math.random() * 10000 + 1);
        window.Server.post(ROOT + 'roulette/Spin.aspx', params, function (data) {
            var r = parseKv(data);
            ...
            animateSpin(winTok, function () {
                ...
                redrawChips();
                setButtons();
                if (balance < minb) { play('lowCredits'); announce('OUT OF CREDITS — refresh to reset', 0); }
            });
        }, function (err) {
            spinning = false;
            setButtons();
    ...
    function tableEventCoords(ev) {
        var rect = $('table-img').getBoundingClientRect();
        var ix = (ev.clientX - rect.left) / rect.width * IMG_W;
        var iy = (ev.clientY - rect.top) / rect.height * IMG_H;
        return { ix: ix, iy: iy };
    }
    ...
    function heartbeat() {
        var params = 'GameSession=' + GAME_SESSION;
        window.Server.post(ROOT + 'Chiptransfer/Heartbeat.aspx', params, function (data) {
            var r = parseKv(data);
