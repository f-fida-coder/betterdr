...
var slotMachine = {
	...
	init: function() {
		...
		$('#soundOffButton').click(function() { slotMachine.toggle_sound(); toggle_MenuMobile();});

		$('#fastPlayButton').click(function() { toggle_fastPlay(); toggle_MenuMobile();});

		$('#show-hide-menu').click(function() { toggle_MenuMobile(); });

		$('#historyButton').click(function() { openHistoryClick(true); toggle_MenuMobile();});
		$('#helpButton').click(function() { openHelpClick(true); toggle_MenuMobile();});
		...
		$('#cashierButton').click(function() { cashierManager.openCashier(); toggle_MenuMobile();});
...
/*
	...
	change_lines : function(delta) {
		...
		if((typeof(Global.BonusRound.brvars) != "undefined" && Global.BonusRound.brvars != null && Global.BonusRound.brvars != "")) {return false; }
	...
	spin: function() {
		...
		if((typeof(Global.BonusRound.brvars) != "undefined" && Global.BonusRound.brvars != null && Global.BonusRound.brvars != "")) {  console.log("esta spining bonus round"); return false; }
		var totalBet = currentLinesBet * Global.Connector.cv;
		if(isInFreeSpins != 0){
			totalBet = 0;
		}
		...
		if(!fastPlay){
 		...
 		animationLinesTime = animationLinesTime;
		iconsAnimTime = iconsAnimTimeOriginal;
		...
		//remove teasers
		$(".reel-container").removeClass("reel-blink");
		...
		var spinData = null;
		window.setTimeout(function(){ FirstReelTimeoutHit = true; if (spinData != null) { slotMachine.fnStopReelsAndEndSpin(spinData); } }, slotMachine.firstReelStopTime);

		LastFSNumber = Global.Connector.frees;
		processSpin();
	},
		...
		if (typeof Global.Connector.cow != "undefined") { 
			...
			if(acumulateTotalWin > 0){
				...
				if(typeof(Global.Connector.frees) != "undefined" && Global.Connector.frees != null && Global.Connector.frees != ""  && Global.Connector.frees == "0"){
					...
					if(typeof(LastFSNumber) != "undefined" && LastFSNumber != null && LastFSNumber == "1")
						checkBigWinAfterFS(acumulateTotalWin);
					else
						completeAfterFS(acumulateTotalWin)
				}
			}else
				$('#lastWin').html(""); 
	...
	_start_reel_spin: function(i, timeOffset) {
		...
		var fnAnimation = function(){
			...
			if (Date.now() < startTime + slotMachine.reelSpeed1Time + timeOffset ) {
				curPos += slotMachine.reelSpeed1Delta / speedFactor;
			} else {
				curPos += slotMachine.reelSpeed2Delta / speedFactor;
			}
	...
	_stop_reel_spin: function(i, outcome) {
		...
		if (outcome != null) {
			...
			elReel.css({ top: finalPosition - (ReelsArr[i-1].length * slotMachine.iconSize) })
				.animate({ top: finalPosition + slotMachine.bounceHeight}, slotMachine.positioningTime / speedFactor, 'linear', function() {
					if(i==5){
						try {
							setTimeout(function(){
								//slotMachine.sounds['spinning'].stop();
					...
					elReel.animate({top: finalPosition}, slotMachine.bounceTime , 'easeOutBack', function(){});
				});
...
function completeAfterFS(acumulateTotalWin){
...
var ReelsCleanResult = new Array();
function getReelsClean(){
	var arrReels = Global.Connector.reels.split('|');
	ReelsCleanResult = new Array();
	...
	for(var i=0; i< arrReels.length; i++){
		...
		if(typeof(tempReel) != "undefined" && tempReel != null & tempReel.length > 0){
			ReelsCleanResult.push(tempReel);
		}
	}
}

function processTeaser(reel){
	var reelIcons = ReelsCleanResult[reel-1];
	
	if(useTeaserW && reelIcons.indexOf('W') >= 0 ){
		wildTeaser(reel);		
	}
}

function wildTeaser(reel){
	if(reel == 1 || reel == 2){

		playTeaserWSound();
		$("#ReelContainer" + reel).addClass("reel-blink");
	}else if(hasHitNumSym(reel, Global.Connector.hits)){
...
function hasHitNumSym(_numSym, _hitsString){
	if(typeof(_hitsString) == 'undefined' || _hitsString == null || _hitsString.length == 0)
		return false;

	var hitInfoArr = _hitsString.split(',');

	if(hitInfoArr == null || hitInfoArr.length == 0)
		return false;

	for(var i = 0; i < hitInfoArr.length; i++){
		var hitData = hitInfoArr[i].split('.');

		if(hitData != null && hitData.length > 0 && hitData[1] != null){
			var numSymHit = hitData[1];

			try{
				var intNum = parseInt(numSymHit[0]);
				if(intNum == _numSym)	
					return true;
			}catch(err){}
		}
...
function initMachine(){
	...
	if(isMobile.any()){
		...
		$("#show-hide-menu").show();
		$(".menu-items-li").addClass("menu-items-li-mobile");
		$("#nav-bar-overlay").addClass("nav-bar-overlay-mobile");
...
function setInitialReels(){
	...
	//no reels info, then it should generated in code
	//no reels info, then it should generated in code
	...
	//check for expanding wild
	if(Global.Connector.reels.indexOf("WWW")>=0){
		var initialReelsTemp = Global.Connector.reels.split('|');

		for(var i =0; i < initialReelsTemp.length; i++){
			if(initialReelsTemp[i]=="WWW"){
				setExpandingW(i);
			}
		}
	}
...
function evaluateExpandingW(){


	if(typeof(Global.BonusRound.morls) != "undefined" && Global.BonusRound.morls != null && Global.BonusRound.morls!= ""){
		setTimeout(function(){
			var morlsArr = Global.BonusRound.morls.split('|');

			for(var i =0; i < morlsArr.length; i++){
				if(morlsArr[i]=="WWW"){
					animateExpandingW(i);
				}
			}
			var afteExp = setTimeout(function(){
				clearTimeout(afteExp);
				afteExp = null;
				evaluteHits();
			}, (fastPlay?1:900));
...
function evaluteHits(){
	...
	for(var i = 0; i < HitsArray.length; i++){
		...
		if($.isNumeric( hitArr[0]) && $.isNumeric( hitArr[2])){
		...
		else if($.isNumeric( hitArr[0]) && !$.isNumeric( hitArr[2]))//bonus round
		{
			var pathNormalLineBR = parseInt(hitArr[0]);
			var pathCoinsBR  = 0;
			var symbolsBR  = hitArr[1];

			if(brHitsArrray == "")
				brHitsArrray = getWinnerLinesByPath(pathNormalLineBR , pathCoinsBR , symbolsBR );
			else
				brHitsArrray += ";" + getWinnerLinesByPath(pathNormalLineBR , pathCoinsBR , symbolsBR );	
		}
		else //FreeSpins
    ...
    var counterHits = 0;
    var WinnerBR = new Array();
    if(brHitsArrray != null && brHitsArrray != "")
    	WinnerBR = brHitsArrray.split(";");
    ...
    linesInterVal = setInterval(function(){ 
    	if(WinnerLines != null && WinnerLines.length > 0 && counterHits < maxHits){
    		...
    		showWinningLines(hitArray[0], hitArray[1], symbols)
    		...
    		counterHits++;
    	}else if(brHitsArrray.length > 0  && !BRCompleted){
    		var icon = "";
    		
    		var linesTotalPaid = ((typeof(Global.Connector.cow) == "undefined" || Global.Connector.cow == null || Global.Connector.cow == "" || !$.isNumeric(Global.Connector.cow)) ? 0 : parseInt(Global.Connector.cow));
    		if(linesTotalPaid > 0){
    			var moneyPaid = linesTotalPaid * Global.Connector.cv;

   				$('#lastWin').html(formatWithThousandsNoDecimalZeros(moneyPaid)); 
    		}
    			
    		hideAllWinnerLines();
    		for(var i= 0; i < WinnerBR.length; i++){

    			var hitArrayBR = WinnerBR[i].split(":");
	    		var pathBR = hitArrayBR[2];
	    		var symbolsBR = hitArrayBR[3];
	    		processHightLightIconsBR(processData, pathBR, symbolsBR);
	    		showAllWinningLines(hitArrayBR[0], hitArrayBR[1], symbolsBR)
	    		icon = symbolsBR[symbolsBR.length -1];
    		}
    		...
    		for(var i = 0; i < WinnerLines.length; i++){
    			...
    			showAllWinningLines(hitArray[0], hitArray[1], hitArray[3]);
    			$(".reel-icon").css('opacity', '1');
    		}
    		...
    		if(specialHitsArrray != ""){
    			var timer5 = setTimeout(function(){
					...
					timer5 = null;
	    			hideAllWinnerLines();
	    			var specialsArr = specialHitsArrray.split(";");
	    			for(var i = 0; i < specialsArr.length; i++){
	    				var hitArr = specialsArr[i].split('.');

	    				if(hitArr[0] == 'S' && hitArr[2].indexOf('FS') >=0){
	    					//highlight icons Scatter
	    					///TEST
	    					//freeSpinCounter = 10;
	    					//Global.Connector.frees = freeSpinCounter;
			    					$(".reel-icon").css('opacity', opacityIconHightLight);
			    					processHightLightIconsFS(processData, hitArr[1]); 
	    					//slotMachine.sounds['win-icon-X'].play();
...
function processHightLightIcons(reelPoss, path, icons){
	...
	for(var i=0; i< totalIcons; i++){
		...
		//$("#reel-"+ (parseInt(i) + 1 )+"-poss-" + elementPos).hide();
	}
	
	showBoderIcon(path, icons, false);

}

function processHightLightIconsBR(reelPoss, path, icons){
	...
	showBoderIcon(path, icons, true);

}

function showBoderIcon(path, icons, isBonusRound){
...
function animateIcon( reel, row, icon){
	...
	/*clearInterval(iconsAnimationsInterval);
	iconsAnimationsInterval = null;
	$('.high-light-icon').css({'background-image': ""})*/
	...
	if(animatedIconsArr.indexOf(icon)>= 0){
		...
		if(iconsAnimationsInterval["#high-light-icon-re"+reel+"-ro"+row] ==  null){
			...
			var tempInterval = setInterval(function(){
				if(imageCounter > animationsFrames){
					imageCounter = 1;
				}
				var bgPos = imageCounter * imageIconSize;

				$("#high-light-icon-re"+reel+"-ro"+row).css('background-position', -(bgPos ) + 'px  0px');
				imageCounter++;

			},(iconsAnimTime/animationsFrames));
...
function showFSCounter(show){
	if(show){
		playFsOpeningSound();
		$("#fs-container").fadeIn(1000); 
		$("#val-fs").text(Global.Connector.frees);
		
	}else{
		playFsCloseSound();
		$("#fs-container").fadeOut(1000, function(){
			$("#val-fs").text("--");
		}); 
...
function showWinningLines(lineNumber, price, symbols){
	hideAllWinnerLines();

	
	$("#line" + lineNumber).show(); 
	if(symbols.toUpperCase().indexOf("Z") < 0){
...
function showAllWinningLines(lineNumber, price, symbols){
	$("#line" + lineNumber).show(); 
	if(symbols.toUpperCase().indexOf("Z") < 0){
	$("#credits-box" + lineNumber).text(price);
	$("#credits-box" + lineNumber).show();
...
function processFreeSpinsInit(){
	if((typeof(Global.Connector.frees) != "undefined" && Global.Connector.frees != null && Global.Connector.frees != "0")){
		...
		if(isInFreeSpins == 0){
			...
			/*$("#top-leyer").stop().animate({opacity: 0.8},200,function(){
				$(this).css({'background-image': "url('images/bg/top-fs.png')"})
               .animate({opacity: 1},{duration:200});
 			});*/
...
//FITO
		if(Global.Connector.cow > 0){
			var totalWon = Global.Connector.cow * Global.Connector.cv;
			if(totalWon > 0){
			
				$('#lastWin').html(formatWithThousandsNoDecimalZeros(totalWon)); 
				//console.log("isInFreeSpins", isInFreeSpins , " --Global.Connector.frees  -- ", Global.Connector.frees)
				if(typeof(Global.Connector.frees) != "undefined" && Global.Connector.frees != null && Global.Connector.frees != ""  && Global.Connector.frees == "0"){
					//slotMachine.sounds['fastpayout'].play();
					slotMachine.sounds['fastpayout'] = SoundManager.PlayAudio("fastpayout", false);

					var baseBalance =  Global.Connector.bal - totalWon;
					var increment =  totalWon / 20.00;


					var animateWinInterval2 = setInterval(function(){ 
						if (baseBalance >= Global.Connector.bal){
							clearInterval(animateWinInterval2);
							animateWinInterval2 = null;
							$('#credits').html(formatWithThousandsPrecisionAndSymbol(Global.Connector.bal));
							try{
								//slotMachine.sounds['fastpayout'].stop();
								SoundManager.StopAudio("fastpayout", slotMachine.sounds['fastpayout']);
...
function processBonusFreeSpinsInit(){
	...
	if((typeof(Global.Connector.frees) != "undefined" && Global.Connector.frees != null && Global.Connector.frees != "0")){
		...
		if(isInFreeSpins == 1){
			/*$("#global").stop().animate({opacity: 0.8},200,function(){
 			...
 			/*$("#top-leyer").stop().animate({opacity: 0.8},200,function(){
				$(this).css({'background-image': "url('images/bg/top-normal.png')"})
               .animate({opacity: 1},{duration:200});
 			});*/
...
function openHelpClick(show){
	if(show){
		//set payout URL
		var callpayoutURL = '';
		if(Global.Connector.cv != lastCalledCoinValue){
			if(payoutURL.indexOf("?") >= 0){
				callpayoutURL = payoutURL+"&GameSession=" + Global.Connector.gameSession + "&coinValue=" + Global.Connector.cv + "&rand="+(Math.random() * 1000000);
			}else{
				callpayoutURL = payoutURL+"?GameSession=" + Global.Connector.gameSession + "&coinValue=" + Global.Connector.cv + "&rand="+(Math.random() * 1000000);
			}		
...
function processAutoPopupCashier(){
	...
	if((typeof(Global.BonusRound.brvars) != "undefined" && Global.BonusRound.brvars != null && Global.BonusRound.brvars != "")) {hasBonusR = true; }

	if ( (Global.Connector.accountId >= 100000000 && Global.Connector.accountId < 200000000) || (Global.Connector.accountId >= 1000000000 && Global.Connector.accountId < 2000000000) ){
		isTournament = true;
	}

	if((Global.Connector.bal <= 0 || (isTournament && typeof(Global.ChipTransfer.tcounter) != "undefined" && Global.ChipTransfer.tcounter != null && Global.ChipTransfer.tcounter != "" && Global.ChipTransfer.tcounter == "0")) && Global.Connector.showCashier && isInFreeSpins == 0 && !hassFreeSpins && !visibleError && !visibleCashier && !hasBonusR){
...
function playExpandingSound(){
	slotMachine.sounds['expW'] = SoundManager.PlayAudio('expW', false);
	readyToPlayBGSound = false;
}
...
function playTeaserWSound(){
	slotMachine.sounds['teaser1'] = SoundManager.PlayAudio('teaser1', false);
}

function playTeaserXSound(){
	slotMachine.sounds['teaser2'] = SoundManager.PlayAudio('teaser2', false);
}


function playFsOpeningSound(){
	slotMachine.sounds['FsOpening'] = SoundManager.PlayAudio('FsOpening', false);
}

function playFsCloseSound(){
	slotMachine.sounds['FsClose'] = SoundManager.PlayAudio('FsClose', false);
}
...
function toggle_MenuMobile(){
	...
	if ($('#show-hide-menu').hasClass("on")) {
        menuIsVisible =false;
        $('#show-hide-menu').html('MENU');
        $('.nav-bar').fadeOut(500);
    } else {
        menuIsVisible =true;
        $('#show-hide-menu').html('HIDE');
        $('.nav-bar').show();
    }
    $('#show-hide-menu').toggleClass("on");
}

function setExpandingW(reel){
	$("#sp-r"+reel).show();
	$("#sp-r"+reel).css({'background-image': "url('images/icons/expandingW/"+expandingWMaxFrame+".png')"});
}



function animateExpandingW(reel){
	if(reel == 1){
		clearInterval(reel1);
		reel1 = null;
...
var reel4 = null;
var reel5 = null;
function animateExpandingR1(){
	if(!fastPlay){
		var frame = 1;
		$("#sp-r1").show();
		$("#sp-r1").css({'background-image': "url('images/icons/expandingW/"+frame+".png')"});
		reel1 = setInterval(function(){
			if(frame >= expandingWMaxFrame){
				$("#sp-r1").css({'background-image': "url('images/icons/expandingW/"+frame+".png')"});
				clearInterval(reel1);
				reel1 = null;
			}else{
		...
		}, (800 / expandingWMaxFrame));
	}else{
		$("#sp-r1").show();
		$("#sp-r1").css({'background-image': "url('images/icons/expandingW/"+expandingWMaxFrame+".png')"});
	}	
}

function animateExpandingR2(){
	if(!fastPlay){
		var frame = 1;
		$("#sp-r2").show();
		$("#sp-r2").css({'background-image': "url('images/icons/expandingW/"+frame+".png')"});
		...
		$("#sp-r2").show();
		$("#sp-r2").css({'background-image': "url('images/icons/expandingW/"+expandingWMaxFrame+".png')"});
	}	
}

function animateExpandingR3(){
	if(!fastPlay){
		var frame = 1;
		$("#sp-r3").show();
		$("#sp-r3").css({'background-image': "url('images/icons/expandingW/"+frame+".png')"});
		...
		$("#sp-r3").show();
		$("#sp-r3").css({'background-image': "url('images/icons/expandingW/"+expandingWMaxFrame+".png')"});
	}	
}

function animateExpandingR4(){
	if(!fastPlay){
		var frame = 1;
		$("#sp-r4").show();
		$("#sp-r4").css({'background-image': "url('images/icons/expandingW/"+frame+".png')"});
		reel4 = setInterval(function(){
			if(frame >= expandingWMaxFrame){
				$("#sp-r4").css({'background-image': "url('images/icons/expandingW/"+frame+".png')"});
				clearInterval(reel4);
				reel4 = null;
		...
		$("#sp-r4").show();
		$("#sp-r4").css({'background-image': "url('images/icons/expandingW/"+expandingWMaxFrame+".png')"});
	}	
}

function animateExpandingR5(){
	if(!fastPlay){
		var frame = 1;
		$("#sp-r5").show();
		$("#sp-r5").css({'background-image': "url('images/icons/expandingW/"+frame+".png')"});
		reel5 = setInterval(function(){
			if(frame >= expandingWMaxFrame){
				$("#sp-r5").css({'background-image': "url('images/icons/expandingW/"+frame+".png')"});
				clearInterval(reel5);
				reel5 = null;
		...
		$("#sp-r5").show();
		$("#sp-r5").css({'background-image': "url('images/icons/expandingW/"+expandingWMaxFrame+".png')"});
	}	
}
function hideAllExpandingW(){
	hideExpandingW(1);
	hideExpandingW(2);
	hideExpandingW(3);
	hideExpandingW(4);
...
function hideExpandingW(reel){
	$("#sp-r" + reel).hide();
	$("#sp-r" + reel).css({'background-image': ""});
}


function processBGAnimation(){

	if(currentBG == 1){
		$(".torch-container-2").fadeOut(3000);
		$("#bg1").show();
		$(".torch-container").fadeIn(3500);
		$("#bg3").fadeOut(3000,function(){
		...
		//$(".torch-container").hide();
		$(".torch-container").fadeOut(2000);
		...
		$(".torch-container-2").fadeIn(3000);
		$("#bg3").fadeIn(3000,function(){
			//$(".torch-container-2").show();
			$("#bg1").hide();
			$(".torch-container").hide();
			$("#bg2").hide();
		});
...
/*big win*/
var animationLinesTimeOriginal = animationLinesTime;
var iconsAnimTimeOriginal = iconsAnimTime;
function checkBigWinNormalSpin(){
	if(btnStopClick){
		console.log("hace el retorno btnStopClick",btnStopClick)
		evaluteHits();
		return;
	}

	$("#stopButton").hide();
	var currentFS = Global.Connector.frees;
	if(typeof(currentFS) =="undefined"|| currentFS == null || currentFS == "")
		currentFS = 0;
	
	if(fastPlay || isInFreeSpins || currentFS > 0)
		evaluteHits();
	else{

		var totalWon = Global.Connector.cow *  Global.Connector.cv;
		var totalCoins  = Global.Connector.cow;

		if( totalCoins >= BigWinMinCoins){
			
			slotMachine.sounds['thunder'] = SoundManager.PlayAudio("thunder", false);
			$("#tund").fadeIn(200, function(){
				$("#tund").fadeOut(200);
			});
			var delay1 = setTimeout(function(){
				clearInterval(delay1);
				delay1 = null;
				//$("#bg-va").html(formatWithThousandsNoDecimalZeros(totalWon));
				startBigWinAnimation();
				slotMachine.sounds['fire'] = SoundManager.PlayAudio("fire", false);
				slotMachine.sounds['horse'] = SoundManager.PlayAudio("horse", false);
				slotMachine.sounds['end1'] = SoundManager.PlayAudio("end", false);
				$("#bg-w").fadeIn(1000, function(){
					
					counterBigWin(totalWon);

					setTimeout(function(){
						slotMachine.sounds['evil'] = SoundManager.PlayAudio("evil", false);
					}, 1500);

					setTimeout(function(){
						stopBigWinAnimation();
						slotMachine.sounds['end'] = SoundManager.PlayAudio("end", false);
						
						$("#bg-w").fadeOut(1000, function(){
							$("#bg-va").html('');
							animationLinesTime = animationLinesTime / 2;
							iconsAnimTime = iconsAnimTimeOriginal /2;
							evaluteHits();
						})
					}, 3000);
				} );
...
function checkBigWinAfterFS(winAmount){

	if(fastPlay)
		completeAfterFS(winAmount);
	else{

		var totalWon = winAmount;
		var totalCoins  = winAmount /  Global.Connector.cv;
...
function counterBigWin(winAmount){

	
	slotMachine.sounds['coins'] = SoundManager.PlayAudio("coins", false);
	var baseBalance =  0;
	var increment =  winAmount / 20.00;


	var BigWinCounterInterval = setInterval(function(){ 
		if (baseBalance >= winAmount){
			clearInterval(BigWinCounterInterval);
			BigWinCounterInterval = null;
			$("#bg-va").html(formatWithThousandsPrecisionAndSymbol(winAmount));
			/*try{
				//slotMachine.sounds['fastpayout'].stop();
				SoundManager.StopAudio("counter-bw", slotMachine.sounds['counter-bw']);
			}catch(er){}*/
		}
		else{
			baseBalance += increment;
			$("#bg-va").html(formatWithThousandsPrecisionAndSymbol(baseBalance));
		}
	}, 50);
}
...
var canvas = document.getElementById("canvas-bw");
...
function setPixel(x, y, v) {
  ...
  var v2 = 255 - v;
  if (y > 55) {
    v2 -= Math.pow((y - 55), 1.45);
    if (v2 < 0) v2 = 0;
  }



  pixels.data[4 * index + 0] = 131;//161;//255;  // R value
  pixels.data[4 * index + 1] = 250;//243;//10;    // G value
  pixels.data[4 * index + 2] = 214;//240;//10;  // B value
  pixels.data[4 * index + 3] = 255 - v2;  // A value
  //pixels.data[4 * index + 3] = 255 - v2;
}
...
for (var x = 0; x < W; ++x) {
  setPixel(x, H - 1, Math.round(Math.random()) * 255 | 0);
}

var framesPerSecond = 35;
var killAnimation = false;
var timerBigWin = null;
function update() {
	if(killAnimation){
		clearTimeout(timerBigWin);
		timerBigWin = null;
		return;
	}

	timerBigWin = setTimeout(function() {
	  for (var x = 0; x < W; ++x) {
	    for (var y = 0; y < H - 1; ++y) {
	      var vv = ((getPixel(x - 1, y + 1) + getPixel(x, y + 1) + getPixel(x + 1, y + 1)) / 3) | 0;
	      vv -= 3;
	      setPixel(x, y, vv);
	    }
	  }
...
function startBigWinAnimation(){
	killAnimation = false;
	requestAnimationFrame(update);
}

function stopBigWinAnimation(){
	killAnimation = true;
