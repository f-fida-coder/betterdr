# Arabian Treasure — offline / embeddable slot
...
```html
<iframe src="/ArabianTreasure/index.html"
...
- **The paytable and paylines are your own.** The vendor's payout table and line
  ...
  ≈ 91%, 30 lines). Override it to match whatever win math you want.
...
window.SLOT_OFFLINE_CONFIG = {
  ...
  paytable: {
    A:{3:45,4:230,5:920}, B:{3:36,4:180,5:700}, C:{3:30,4:145,5:520},
    D:{3:26,4:125,5:420}, E:{3:21,4:100,5:340}, F:{3:16,4:72,5:250},
...
- **Spin** (`lb`, `cv`) → `reels=|r1|r2|r3|r4|r5|` (each three symbols, top→bottom),
  `cow` (total coins won), `hits` (per-line wins), `bal`, `gid`, `frees`, `iserr=0`.
- **hit entry format:** `<path>.<count><symbol>.<coins>` — e.g. `22222.5A.920`,
  where `path` is the row (1–3) the line uses on each matched reel, left to right.

Symbols: `A`–`H` pay on lines, `W` = wild (substitutes), `X` = scatter.
Total bet = `lb × cv`; total win = `cow × cv`.

## Notes

- Balance persists in `localStorage` (`slot_al_*`); set `persist:false` for a
