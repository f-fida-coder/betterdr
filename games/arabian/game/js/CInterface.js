function CInterface(iCurBet,iTotBet,iMoney){
	...
	_oMoneyText = new CTLText(s_oAttachSection, 
                    ...
                    0, 0,
                    TEXT_MONEY +"\n"+Math.ceil(iMoney)+TEXT_CURRENCY,
                    true, true, true,
                    false );
    ...
    this.refreshBet = function(iBet){
        _oCoinText.refreshText(Math.ceil(iBet));
    };

    this.refreshTotalBet = function(iTotBet){
        _oTotalBetText.refreshText(TEXT_BET +": "+Math.ceil(iTotBet));
    };
    ...
    this.refreshWinText = function(iWin){
        _oWinText.refreshText(TEXT_WIN + " "+Math.ceil(iWin));
    };
