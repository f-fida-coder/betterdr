...
final class AgentController
{
    ...
    private function createUser(): void
    {
        try {
            ...
            if (count($duplicateMatches) > 0) {
                $duplicateWarningPayload = $this->buildDuplicatePlayerResponse($firstName, $lastName, $fullName, $phoneNumber, $email, $duplicateMatches);
            }
    ...
    /**
     * Aggregate each downline agent's house cut (net collections × cut%)
    ...
    private function getAgentCuts(): void
    {
        try {
            ...
            if ($periodType === 'week') {
                // Week starts Tuesday (matches getDownlineSummary / settlement)
            ...
            // 2. Build userId → agentId map by walking users of each agent in the tree
            $allAgentObjectIds = [];
            foreach (array_keys($allAgents) as $aid) {
                if (preg_match('/^[a-f0-9]{24}$/i', $aid) === 1) {
                    $allAgentObjectIds[] = SqlRepository::id($aid);
            ...
            // 3. One pass over ALL deposit/withdrawal transactions for these users.
            //    Bucket per agent into (a) period net, (b) lifetime net.
            $periodNetByAgent = [];
            $lifetimeNetByAgent = [];
            if (count($userIdToAgentId) > 0) {
                $userObjectIds = [];
                ...
                if (count($userObjectIds) > 0) {
                    ...
                    foreach ($allTx as $tx) {
                        ...
                        if ($refType === 'AgentFunding') {
                            continue; // agent-to-house funding is not a player collection
                        }
                        $txUserId = (string) ($tx['userId'] ?? '');
                        if ($txUserId === '') continue;
                        $uAgentId = $userIdToAgentId[$txUserId] ?? '';
                        ...
                        if ($periodStartMs !== null) {
                            ...
                            // lifetime period — period total == lifetime
            ...
            // 4. Build the flat agent list (only role=agent; masters roll up to their agents)
            $HOUSE_CUT_PCT = 5.0;
            $flatAgents = [];
            ...
            usort($flatAgents, function ($a, $b) {
                $cmp = ($b['periodAmount'] ?? 0) <=> ($a['periodAmount'] ?? 0);
                if ($cmp !== 0) return $cmp;
                return strcasecmp((string) ($a['username'] ?? ''), (string) ($b['username'] ?? ''));
            });
    ...
    private function extractTxMillis($raw): ?int
    {
        ...
        if (is_array($raw)) {
            if (isset($raw['$date'])) {
                return $this->extractTxMillis($raw['$date']);
            }
            if (isset($raw['date'])) {
                return $this->extractTxMillis($raw['date']);
            }
