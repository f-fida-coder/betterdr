...
const MatchCard = React.memo(({ match, oddsFormat, onAddToSlip, selectedKeys, visibleMarkets, marketCount, onToggleFavorite, teaserPoints = 0, isTeaserMode = false, betMode = 'straight' }) => {
    ...
    // Some sports hide the Spread⇄Alt toggle and use Buy Points instead (WNBA:
    // alt spreads dropped at ingestion; football: Buy Points, not alt ladders).
    // Gate the toggle off by sport so the Spread column stays a static label.
    // Team totals (the Total⇄TT toggle) are unaffected.
    ...
    return (
        <div style={matchCardStyle}>
            ...
            {/* Combined header row: live dot + date on the left,
                ...
                columns below. Trailing empty slot matches the vertical
                Props column on the right (dropped in teaser mode since
            ...
            <div style={{
                ...
                {/* Right-column Props button — spans both team rows so it
                    ...
                    consolidated "Props" entry (the old "+" More Bets + "P+"
                    builder merged 2026-07-08): opens PropBuilderModal with
                    includeGameMarkets. The label runs VERTICALLY (top-to-
                    bottom) so the column stays narrow without clipping.
                    Hidden in teaser mode — teasers can only combine
            ...
            {/* 3-way moneyline Draw row (soccer / 1X2). Renders only when the
                ...
                No divider line — the row sits tight against the two team
                rows (marginTop -4 cancels the grid's 8px bottom padding so
                the gap matches the 4px row-gap between Away/Home), with a
                small 8px gap before the notice divider below. PO follow-up
                2026-07-07 (supersedes the earlier same-day breathing-room
                note): Draw belongs visually inside the game block; the
                small gap goes below it, not above. Tagged 'h2h' (NOT
