...
const SportContentView = ({ sportId, selectedItems = [], filter = null, status = 'live-upcoming', activeBetMode = 'straight', limit = 0 }) => {
    ...
    const showSpread = ['straight', 'parlay', 'open_parlay', 'teaser', 'if_bet', 'reverse', 'round_robin'].includes(normalizedMode);
    const showMoneyline = ['straight', 'parlay', 'open_parlay', 'if_bet', 'reverse', 'round_robin'].includes(normalizedMode);
    const showTotals = ['straight', 'parlay', 'open_parlay', 'teaser', 'if_bet', 'reverse', 'round_robin'].includes(normalizedMode);
                                    ...
                                    <div className="match-odds">
                                        <div className="odds-row">
                                            {showSpread && (() => {
                                                ...
                                                // Some sports hide the Spread⇄Alt toggle and use Buy
                                                // Points instead (WNBA: alt spreads dropped at ingestion;
                                                // football: Buy Points, not alt ladders). Gate the toggle
                                                // off by sport so the Spread column stays a static label.
                                                // Team totals (Total⇄TT) are unaffected.
                                                const hasAltSpreads = !isAltSpreadSuppressedSport(match.sportKey) && (awayLadder.length > 0 || homeLadder.length > 0);
