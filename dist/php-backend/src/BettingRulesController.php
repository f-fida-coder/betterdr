...
final class BettingRulesController
{
    ...
    private function getPublicBetModeRules(): void
    {
        try {
            ...
            // Inject teaser fields the frontend needs: the structured
            // `teaserTypes` catalog drives the new type-picker landing
            // screen; legacy `teaserPointOptionsBySport` and the flat
            // `teaserPointOptions` are kept populated so older clients
            // keep working. All three derive from the same source
            // (BetModeRules::DEFAULT_TEASER_TYPES) when the DB row was
            // seeded by the new code path; if the DB row predates
            // teaserTypes (legacy seed), we hydrate from defaults so
            // the frontend never sees a missing catalog.
