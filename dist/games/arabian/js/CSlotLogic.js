...
function checkLogin(){
    // Local login/session bootstrap is disabled: balance is always fetched from parent bridge.
    if(typeof tryCheckLogin === "function"){
        tryCheckLogin();
    }
    return "res=false&error=LOCAL_LOGIN_DISABLED";
}

function callSpin(iNumBettingLines,iCoin,iCurBet){
    // Local settlement is disabled: spins are always settled by backend via parent bridge.
    if(typeof tryCallSpin === "function"){
        tryCallSpin(iCoin,iCurBet,iNumBettingLines);
    }
    return "res=false&error=LOCAL_SETTLEMENT_DISABLED";
}
