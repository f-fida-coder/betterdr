import{b as d,j as e}from"./vendor-common-DIOJvOBD.js";import{m as Qe,w as Te,t as Ee,x as Re,D as me,au as Ct,av as kt,N as He,aw as Ge,ax as It,o as Pt}from"./app-api-B_Kx2orW.js";import{n as Lt,G as Ve,H as Mt,I as Ut}from"./utils-shared-DSg3yjz0.js";const Be=s=>{const l=String(s||"").toUpperCase().replace(/[^A-Z0-9]/g,"");if(!l)return"";const f=l.match(/^[A-Z]+/);return f&&f[0]?f[0]:l.replace(/\d+$/,"")||l},Tt=s=>s?`FREEPLAY
...
This account starts with $0 in freeplay. If freeplay is added later, you must click "Use your freeplay balance $" when placing a bet. Freeplay is limited to straight bets only and no parlays.`,Et=new Set(["admin","agent","master_agent","super_agent"]),Rt=s=>!Et.has(String(s?.role||"").trim().toLowerCase()),K=s=>String(s||"").trim().toLowerCase(),O=s=>String(s||"").trim(),Bt=new Set(["admin","agent","master_agent","super_agent"]),Ne=s=>String(s?.nodeType||"").trim().toLowerCase()==="player"?!1:Bt.has(K(s?.role)),Z=s=>{const l=K(s?.role);return l==="master_agent"||l==="super_agent"},je=s=>K(s?.role)==="agent",Ze=s=>{if(!Ne(s))return null;const l=Array.isArray(s.children)?s.children.map(f=>Ze(f)).filter(Boolean):[];return{...s,id:O(s.id),children:l}},ue=(s,l)=>{const f=O(l);if(!f||!s)return null;if(O(s.id)===f)return s;const h=Array.isArray(s.children)?s.children:[];for(const j of h){const v=ue(j,f);if(v)return v}return null},$e=(s,l)=>{const f=O(l);if(!f||!s)return[];const h=O(s.id);if(h===f)return[h];const j=Array.isArray(s.children)?s.children:[];for(const v of j){const I=$e(v,f);if(I.length>0)return[h,...I]}return[]},_e=(s,l,f=!0,h=0,j=[])=>(s&&((f||h>0)&&l(s,h)&&j.push(s),(Array.isArray(s.children)?s.children:[]).forEach(I=>_e(I,l,!0,h+1,j))),j),$t=s=>{const l=K(s?.role);return l==="master_agent"?"MASTER":l==="super_agent"?"SUPER":l==="agent"?"AGENT":l==="admin"?"ADMIN":l?l.replace(/_/g," ").toUpperCase():"ACCOUNT"},_t=s=>K(s?.role).replace(/_/g,"-")||"account",Ft=s=>{const l=String(s?.username||"").toLowerCase(),h=K(s?.role).replace(/_/g," ");return`${l} ${h}`.trim()},Fe=(s,l)=>{const f=String(l||"").trim().toLowerCase();return!f||Ft(s).includes(f)?!0:(Array.isArray(s?.children)?s.children:[]).some(h=>Fe(h,f))};function zt({rootNode:s,loading:l=!1,error:f="",searchQuery:h="",onSearchQueryChange:j,expandedNodes:v,onToggleNode:I,onSelectNode:J,onSelectDirect:B,selectedNodeId:N="",directSelected:U=!1,selectionMode:k="player",searchPlaceholder:P="Search accounts...",emptyLabel:ge="No matching accounts"}){const q=String(h||"").trim().toLowerCase(),fe=q!==""||l||f,X=(w,W=0,$=!1)=>{if(!w||!Ne(w)||k==="master"&&!$&&!Z(w)||q&&!Fe(w,q))return null;if(k==="player"&&!je(w)){const ae=(Array.isArray(w.children)?w.children:[]).filter(Ne).map(oe=>X(oe,W,!1));return ae.some(Boolean)?e.jsx(e.Fragment,{children:ae}):null}const L=O(w.id),Y=(Array.isArray(w.children)?w.children:[]).filter(z=>Ne(z)&&(k!=="master"||Z(z))),T=Y.length>0&&($||Z(w)),H=q?!0:v.has(L),ie=k==="player"?je(w):$?typeof B=="function":Z(w),le=$?U:N!==""&&N===L;return e.jsxs("div",{className:"assignment-tree-branch",children:[e.jsxs("div",{className:`tree-node ${$?"root-node":""} assignment-tree-row ${le?"selected":""} ${ie?"selectable":""}`,style:$?void 0:{paddingLeft:`${16+W*20}px`},children:[e.jsx("button",{type:"button",className:`assignment-tree-toggle-btn ${T?"":"is-spacer"}`,onClick:()=>{T&&I?.(L)},"aria-label":T?H?"Collapse branch":"Expand branch":"No child accounts",disabled:!T,children:T?H?"−":"+":""}),e.jsxs("button",{type:"button",className:"assignment-tree-node-btn",onClick:()=>{if(ie){if($&&typeof B=="function"){B(w);return}J?.(w);return}T&&I?.(L)},children:[e.jsx("span",{className:"node-name",children:String(w.username||"").toUpperCase()}),e.jsx("span",{className:`node-role-badge role-${_t(w)}`,children:$t(w)})]})]}),T&&H&&Y.length>0&&e.jsx("div",{className:"node-children assignment-tree-children",children:Y.map(z=>X(z,W+1,!1))})]},`${L||"root"}-${W}`)},se=!!s&&Fe(s,q);return e.jsxs("div",{className:"assignment-tree-picker",children:[e.jsxs("div",{className:"search-pill assignment-tree-search-pill",children:[e.jsx("span",{className:"pill-label",children:"Tree"}),e.jsx("input",{type:"text",placeholder:P,value:h,onChange:w=>j?.(w.target.value)})]}),fe&&e.jsx("div",{className:"assignment-tree-results-dropdown",children:e.jsx("div",{className:"tree-scroll-area assignment-tree-scroll-area",children:l?e.jsx("div",{className:"tree-loading",children:"Loading hierarchy..."}):f?e.jsx("div",{className:"tree-error",children:f}):se?X(s,0,!0):e.jsx("div",{className:"tree-loading",children:ge})})}),e.jsx("style",{children:`
