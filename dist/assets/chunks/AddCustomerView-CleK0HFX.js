import{r as d,j as e}from"./vendor-react-Bd2HVIcR.js";import{x as Qe,E as Te,B as Ee,F as Be,K as me,aH as Ct,aI as kt,X as He,aJ as Ve,aK as It,y as Pt}from"./app-api-DeiNQoe_.js";import{ac as Lt,as as Ge,at as Mt,au as Ut}from"./utils-shared-DrPilpc8.js";import"./vendor-common-_eGvg4ul.js";import"./dashboard-views-CUG8NH2s.js";import"./scoreboard-views-Cg6cmMah.js";import"./contexts-shared-Dy2sUuKn.js";const Re=s=>{const l=String(s||"").toUpperCase().replace(/[^A-Z0-9]/g,"");if(!l)return"";const f=l.match(/^[A-Z]+/);return f&&f[0]?f[0]:l.replace(/\d+$/,"")||l},Tt=s=>s?`FREEPLAY
...
This account starts with $0 in freeplay. If freeplay is added later, you must click "Use your freeplay balance $" when placing a bet. Freeplay is limited to straight bets only and no parlays.`,Et=new Set(["admin","agent","master_agent","super_agent"]),Bt=s=>!Et.has(String(s?.role||"").trim().toLowerCase()),Z=s=>String(s||"").trim().toLowerCase(),D=s=>String(s||"").trim(),Rt=new Set(["admin","agent","master_agent","super_agent"]),Ne=s=>String(s?.nodeType||"").trim().toLowerCase()==="player"?!1:Rt.has(Z(s?.role)),K=s=>{const l=Z(s?.role);return l==="master_agent"||l==="super_agent"},je=s=>Z(s?.role)==="agent",Ke=s=>{if(!Ne(s))return null;const l=Array.isArray(s.children)?s.children.map(f=>Ke(f)).filter(Boolean):[];return{...s,id:D(s.id),children:l}},ue=(s,l)=>{const f=D(l);if(!f||!s)return null;if(D(s.id)===f)return s;const x=Array.isArray(s.children)?s.children:[];for(const j of x){const v=ue(j,f);if(v)return v}return null},$e=(s,l)=>{const f=D(l);if(!f||!s)return[];const x=D(s.id);if(x===f)return[x];const j=Array.isArray(s.children)?s.children:[];for(const v of j){const I=$e(v,f);if(I.length>0)return[x,...I]}return[]},Fe=(s,l,f=!0,x=0,j=[])=>(s&&((f||x>0)&&l(s,x)&&j.push(s),(Array.isArray(s.children)?s.children:[]).forEach(I=>Fe(I,l,!0,x+1,j))),j),$t=s=>{const l=Z(s?.role);return l==="master_agent"?"MASTER":l==="super_agent"?"SUPER":l==="agent"?"AGENT":l==="admin"?"ADMIN":l?l.replace(/_/g," ").toUpperCase():"ACCOUNT"},Ft=s=>Z(s?.role).replace(/_/g,"-")||"account",_t=s=>{const l=String(s?.username||"").toLowerCase(),x=Z(s?.role).replace(/_/g," ");return`${l} ${x}`.trim()},_e=(s,l)=>{const f=String(l||"").trim().toLowerCase();return!f||_t(s).includes(f)?!0:(Array.isArray(s?.children)?s.children:[]).some(x=>_e(x,f))};function zt({rootNode:s,loading:l=!1,error:f="",searchQuery:x="",onSearchQueryChange:j,expandedNodes:v,onToggleNode:I,onSelectNode:J,onSelectDirect:R,selectedNodeId:N="",directSelected:U=!1,selectionMode:k="player",searchPlaceholder:P="Search accounts...",emptyLabel:ge="No matching accounts"}){const q=String(x||"").trim().toLowerCase(),fe=q!==""||l||f,X=(w,W=0,$=!1)=>{if(!w||!Ne(w)||k==="master"&&!$&&!K(w)||q&&!_e(w,q))return null;if(k==="player"&&!je(w)){const ae=(Array.isArray(w.children)?w.children:[]).filter(Ne).map(oe=>X(oe,W,!1));return ae.some(Boolean)?e.jsx(e.Fragment,{children:ae}):null}const L=D(w.id),Y=(Array.isArray(w.children)?w.children:[]).filter(z=>Ne(z)&&(k!=="master"||K(z))),T=Y.length>0&&($||K(w)),H=q?!0:v.has(L),ie=k==="player"?je(w):$?typeof R=="function":K(w),le=$?U:N!==""&&N===L;return e.jsxs("div",{className:"assignment-tree-branch",children:[e.jsxs("div",{className:`tree-node ${$?"root-node":""} assignment-tree-row ${le?"selected":""} ${ie?"selectable":""}`,style:$?void 0:{paddingLeft:`${16+W*20}px`},children:[e.jsx("button",{type:"button",className:`assignment-tree-toggle-btn ${T?"":"is-spacer"}`,onClick:()=>{T&&I?.(L)},"aria-label":T?H?"Collapse branch":"Expand branch":"No child accounts",disabled:!T,children:T?H?"−":"+":""}),e.jsxs("button",{type:"button",className:"assignment-tree-node-btn",onClick:()=>{if(ie){if($&&typeof R=="function"){R(w);return}J?.(w);return}T&&I?.(L)},children:[e.jsx("span",{className:"node-name",children:String(w.username||"").toUpperCase()}),e.jsx("span",{className:`node-role-badge role-${Ft(w)}`,children:$t(w)})]})]}),T&&H&&Y.length>0&&e.jsx("div",{className:"node-children assignment-tree-children",children:Y.map(z=>X(z,W+1,!1))})]},`${L||"root"}-${W}`)},se=!!s&&_e(s,q);return e.jsxs("div",{className:"assignment-tree-picker",children:[e.jsxs("div",{className:"search-pill assignment-tree-search-pill",children:[e.jsx("span",{className:"pill-label",children:"Tree"}),e.jsx("input",{type:"text",placeholder:P,value:x,onChange:w=>j?.(w.target.value)})]}),fe&&e.jsx("div",{className:"assignment-tree-results-dropdown",children:e.jsx("div",{className:"tree-scroll-area assignment-tree-scroll-area",children:l?e.jsx("div",{className:"tree-loading",children:"Loading hierarchy..."}):f?e.jsx("div",{className:"tree-error",children:f}):se?X(s,0,!0):e.jsx("div",{className:"tree-loading",children:ge})})}),e.jsx("style",{children:`
